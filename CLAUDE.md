...
│   ├── highlight.js           ← 例文中の対象単語マッチ(app.jsとvalidate.jsで共用)
...
| `icon` | string \| null | [Lucide](https://lucide.dev)のアイコン名。**現UIでは未使用**のため無理に探さず `null` でよい(将来用の予約フィールド) |
...
8. **例文中の対象単語**: 原形または規則活用(-s/-es/-ed/-d/-ing、e脱落形、y→ies/ied)で含めること。
   validate.js は `js/highlight.js`(アプリ本体と同一ロジック)で穴埋め生成可否を検証するため、
   不規則活用のみ(例: buy→bought)で使うとエラーになる

## PWA運用ルール

- **シェル資産(HTML/CSS/JS/フォント/アイコン)を変更したら、必ず `sw.js` の `SHELL_CACHE` をバンプする**(v3→v4→…)。
  忘れると Service Worker が更新されず、古いアプリがキャッシュから配信され続ける
- 新しいJS/CSS/フォントファイルを追加したら `sw.js` の `SHELL_ASSETS` にも追加する
